...
The socket is the API between the application layer and the transport layer. The application developer controls everything on the application side of the socket, but has little control on the transport side (choice of protocol, and some buffer/segment size parameters).
...
- **No ordering guarantee**: messages may arrive out of order. No congestion control — UDP can send at any rate regardless of network state. Firewalls often block UDP, which is why some applications (e.g., Internet telephony) fall back to TCP.
