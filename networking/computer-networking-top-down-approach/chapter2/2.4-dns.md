...
| **Load distribution** | A single hostname maps to a set of IP addresses (replicated servers). DNS rotates the ordering of addresses in each reply, distributing traffic across servers. When DNS returns multiple A records for a name, it rotates the ordering each response. Clients typically use the first IP address in the list, so rotation distributes load across replicated servers. CDNs such as Akamai use DNS in more sophisticated ways for content distribution. |
