...
**Context**: Web, e-mail, and DNS all use client-server architectures with significant reliance on always-on infrastructure servers. P2P architectures have minimal (or no) reliance on always-on infrastructure servers. Pairs of intermittently connected hosts (peers — PCs, laptops, smartphones controlled by users, not service providers) communicate directly with each other.

**Use case**: Distributing a large file (e.g., a new Linux OS version, a software patch, an MPEG video) from a single server to a large number of hosts. In client-server distribution the server must send every peer a copy, placing an enormous burden on it. In P2P, each peer can redistribute any portion it has received to any other peer, assisting the server.

As of 2020, the most popular P2P file distribution protocol is **BitTorrent**, originally developed by **Bram Cohen**. Many independent BitTorrent clients conform to the protocol, analogous to multiple Web browsers conforming to HTTP.

### 1.1 Client-Server vs P2P Distribution Time

A clean model for comparing architectures. Setup (Figure 2.22): server and peers connected to the Internet with access links. Simplifying assumptions: abundant core bandwidth (all bottlenecks in access networks), and neither server nor peers participate in other applications (full upload/download bandwidth devoted to the transfer).

**Variables**:
- `F` — file size (bits)
- `N` — number of peers that want a copy
- `u_s` — server upload rate (access link)
- `u_i` — upload rate of peer `i`'s access link
- `d_i` — download rate of peer `i`'s access link
- `d_min` = min{d_1, d_2, ..., d_N} — lowest peer download rate

**Client-Server lower bound (Equation 2.1)**:
...
Derivation:
- The server must transmit NF bits total (one full file to each peer). At rate `u_s`, this takes at least `NF/u_s`.
- The peer with the lowest download rate cannot receive F bits in less than `F/d_min`.

- `NF/u_s`: server must send the full file N times; only the server uploads.
- `F/d_min`: slowest peer determines the floor.
- For large N, `NF/u_s` dominates → **linear growth with N**. For example, if peers increase a thousand-fold (from 1,000 to 1,000,000), distribution time increases by 1,000. Unbounded.
...
**P2P lower bound (Equations 2.2 / 2.3)**:

```
D_p2p = max(F/u_s, F/d_min, NF/(u_s + Σu_i))
```

Derivation (three observations):
1. At the start only the server has the file. It must send each bit at least once into its access link → minimum time at least `F/u_s`. (Unlike client-server, a bit sent once by the server need not be re-sent, as peers can redistribute it.)
2. The peer with the lowest download rate cannot receive F bits in less than `F/d_min`.
3. Total upload capacity of the system: `u_total = u_s + u_1 + ... + u_N`. The system must deliver NF bits total. This cannot happen faster than `u_total`, so minimum time is at least `NF/(u_s + Σu_i)`.

Note: Equation 2.2 is a lower bound assuming bits can be redistributed instantly. In reality chunks (not individual bits) are redistributed, so Equation 2.3 (same formula, used as actual minimum) is a good approximation [Kumar 2006].

- `F/u_s`: server must seed at least one copy into the network.
- `F/d_min`: same floor as client-server.
...
**Key result (Figure 2.23)**: Setting F/u = 1 hour, u_s = 10u (server is 10× a peer), and d_min ≥ u_s (download rates not the bottleneck): the client-server distribution time grows linearly and without bound as N increases. The P2P minimum distribution time is not only always less than client-server — it stays **below 1 hour for any number of peers N**. P2P is *self-scaling*: every new consumer is also a new contributor (redistributor), so each new peer adds upload capacity to the denominator.

### 1.2 BitTorrent Mechanics (Textbook Treatment)

**Terminology**: The collection of all peers participating in the distribution of a particular file is called a **torrent**. A torrent may have fewer than 10 or more than 1,000 peers at any instant.

**Bootstrap**:
1. New peer registers with the **tracker** (infrastructure node per torrent that keeps track of participating peers).
2. Tracker returns a random subset (~50 peers) from the currently participating set.
3. New peer attempts concurrent TCP connections to all 50 → successful connections = **neighboring peers**.
4. Neighbors fluctuate over time as peers join/leave and new peers attempt to connect to Alice.
...
- Peer starts with no chunks; accumulates over time while simultaneously uploading chunks to other peers.
- Periodically, Alice asks each of her L neighboring peers for their list of chunks, then issues requests for chunks she does not have.
- Once complete, peer may leave (selfish) or remain and continue uploading (altruistic). A peer may also leave at any time with only a subset of chunks and later rejoin the torrent.

**Rarest first**:
- From chunks Alice does not have, identify which are rarest among her neighbors (fewest copies among neighbors).
- Request those first.
- Effect: rare chunks get redistributed quickly → (roughly) equalizes copy counts across the torrent.

**Tit-for-tat / Unchoking**:
- Alice measures the incoming bit rate from each neighbor continuously.
- **Top 4 (unchoked)**: the 4 neighbors feeding bits at the highest rate — Alice reciprocates by sending chunks to them. Recalculated every 10 seconds.
- **Optimistic unchoke**: every 30 seconds, pick 1 additional neighbor at random (regardless of rate) and send chunks to it. This peer is "optimistically unchoked."
- All other neighbors: **choked** — receive no chunks from Alice.
- The incentive mechanism is referred to as **tit-for-tat** [Cohen 2003].

**Why it works**:
- Optimistic unchoke lets new peers (who have nothing to trade yet) receive chunks, giving them something to offer.
- If the optimistically unchoked peer (e.g., Bob) starts sending back at a high rate, he may earn a top-4 slot with Alice, and she in turn a top-4 slot with him — they converge on mutual trading.
- Peers with compatible upload rates tend to find each other.
- Every 30 seconds Alice randomly picks a new partner and initiates trading; if both are satisfied, they stay in each other's top-4 until one finds better.
- Free-riders (download only) stay choked by everyone and get poor performance.

**Additional mechanisms** (noted by textbook but not detailed): pieces (mini-chunks), pipelining, random first selection, end-game mode, and anti-snubbing [Cohen 2003].

The textbook notes the tit-for-tat incentive scheme can be circumvented [Liogkas 2006; Locher 2006; Piatek 2008], but the BitTorrent ecosystem remains wildly successful with millions of simultaneous peers actively sharing files.
