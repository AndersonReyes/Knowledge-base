...
**HTTP/1.1 workaround:** browsers open up to 6 parallel TCP connections per host — circumvents HOL blocking but "cheats" TCP congestion control. TCP congestion control aims to give each TCP connection sharing a bottleneck link an equal share (approximately 1/n of bandwidth for n connections), so a browser with 6 parallel connections grabs ~6× the share of a single connection.

**HTTP/2's goal:** eliminate (or reduce) parallel TCP connections for a single Web page. This reduces open sockets at servers AND allows TCP congestion control to operate as intended (fair sharing).
...
> "The ability to break down an HTTP message into independent frames, interleave them, and then reassemble them on the other end is the **single most important enhancement** of HTTP/2." — Kurose & Ross

The **framing sub-layer** handles this:
- When a server sends an HTTP response, the framing sub-layer breaks it into frames: the **header field** → one frame; the **body** → one or more additional frames
- These frames are interleaved with frames from other responses on the single persistent TCP connection
- At the client, frames are first **reassembled** into the original response messages at the framing sub-layer, then processed by the browser as usual
- **Client HTTP requests** are also broken into frames and interleaved (not just server responses)
- Frames are **binary encoded** (more efficient to parse, slightly smaller, less error-prone than ASCII text)
...
**Prioritization:**
- When a client sends concurrent requests to a server, it can prioritize responses by assigning a **weight between 1 and 256** to each message — higher number = higher priority
- The framing sub-layer organizes messages into parallel streams; the server sends frames for highest-priority responses first
- The client also states each message's **dependency** on other messages by specifying the ID of the message it depends on

**Server Push:**
- A server can send **multiple responses for a single client request**: in addition to the response to the original request, the server can push additional objects the client will need, without the client having to explicitly request each one
- The server analyzes the HTML base page (which indicates all objects needed to fully render the page), identifies those objects, and sends them to the client before receiving explicit requests
- Server push eliminates the extra latency due to waiting for the client to discover and request each sub-resource
