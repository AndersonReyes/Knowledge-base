...
Without cache:
```
Traffic intensity on access link = (15 req/s × 1 Mbit/req) / 15 Mbps = 1.0
→ Delay grows without bound (minutes)
```

Option 1 — Upgrade access link to 100 Mbps:
```
Traffic intensity = 0.15 → negligible queuing delay
Total response time ≈ 2 seconds (Internet delay dominates)
```

Option 2 — Install cache with 0.4 hit rate:
```
40% of requests served from cache in ~10ms
Remaining 60% go to Internet:
  Traffic intensity = 0.6 × (15 req/s × 1 Mbit) / 15 Mbps = 0.6 → tens of ms delay

Average response time = 0.4 × 0.01s + 0.6 × 2.01s ≈ 1.2 seconds
```

Cache option provides faster response than the expensive link upgrade.
...
HTTP/3 is designed to run over **QUIC** — a transport protocol implemented in the application layer over UDP. QUIC provides:
- Message multiplexing (per-stream, avoiding TCP's stream-level HOL blocking)
- Per-stream flow control
- Low-latency connection establishment

Many HTTP/2 features are subsumed by QUIC, enabling a simpler HTTP/3 design. As of 2020, HTTP/3 was still in Internet Draft status (not yet fully standardized).
...
|---------|----------|----------|--------|
| Connections | Non-persistent | Persistent + pipelining | Multiplexed (single connection) |
| HOL blocking | Per-connection | Per-connection | Mitigated via framing |
| Parallel TCP connections | N/A | Up to ~6 (browser workaround) | Not needed |
| Header encoding | ASCII text | ASCII text | Binary (HPACK compression) |
| Server push | No | No | Yes |
| Request prioritization | No | No | Yes (weights + dependencies) |
| RFC | 1945 | 7230 | 7540 |
